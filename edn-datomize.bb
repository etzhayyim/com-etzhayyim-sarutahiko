...
;; (com-etzhayyim-sarutahiko 用に superproject manifest/edn-datomize.bb から
;;  移植・適応: schema-path をこのリポジトリ root 直下 schema.edn に変更し、
;;  既存キーが既に意味のある名前空間を持つファイル向けに
;;  wrap-map-preserve-ns! を追加した。)
;;
...
;;
;; 使い方:
;;   bb edn-datomize.bb wrap-map <path> <ns>       — map 1個のファイルを変換（bare key に <ns> を付与）
;;   bb edn-datomize.bb wrap-map-preserve-ns <path> <fallback-ns>
;;                                                   — map 1個のファイルを変換。
;;     既に名前空間付きのキー（(namespace k) 非nil）はそのまま保持し、
;;     bare key だけ <fallback-ns> を付与する（idiomatic namespaced-key ファイル用）。
...
(defn preserve-or-namespace-key
  "既に名前空間付き(namespace k が非nil)ならそのまま。bare key だけ fallback-ns を付与する。"
  [fallback-ns k]
  (if (namespace k) k (keyword fallback-ns (name k))))
...
(defn entity-from-map-preserve-ns
  "トップレベル map を 1 entity にする。既に名前空間付きのキーはそのまま保持し、
   bare key だけ fallback-ns を付与する。"
  [content fallback-ns]
  (into {:db/id -1}
        (map (fn [[k v]] [(preserve-or-namespace-key fallback-ns k) (attr-value v)]))
        content))
...
(defn schema-attrs-preserve-ns
  ...
  (for [[k v] content]
    (let [{:keys [type card]} (classify v)]
      {:db/ident (preserve-or-namespace-key fallback-ns k)
       :db/valueType type
       :db/cardinality card})))
...
(defn wrap-map-preserve-ns! [rel-path fallback-ns]
  (let [f (io/file root rel-path)
        content (slurp-edn f)]
    (if (already-tx-data? content)
      (println "skip (already tx-data):" rel-path)
      (let [entity (entity-from-map-preserve-ns content fallback-ns)
            attrs (schema-attrs-preserve-ns content fallback-ns)]
        (spit f (pr-str [entity]))
        (merge-schema! attrs)
        (println "wrapped (ns-preserving)" rel-path "->" (count entity) "attrs, fallback-ns=" fallback-ns)))))
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   ...
                   (merge-schema! (:attrs @report))
                   (println @report))
      (do (println "usage: bb edn-datomize.bb [wrap-map <path> <ns> | wrap-map-preserve-ns <path> <fallback-ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
